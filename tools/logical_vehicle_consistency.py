...
def associate_tracklets(
    ...
    for from_id in sorted(by_from, key=lambda item: by_id[item].end_frame):
        ...
        if best["to_tracklet_id"] in used_to:
            conflict = dict(best)
            conflict["link_status"] = "REJECT_USED_TO_CONFLICT"
            conflict["review_status"] = "PENDING_REVIEW"
            ambiguous_links.append(conflict)
            continue
...
def build_identity_purity_report(logical_rows: list[dict], accepted_cross_raw_recoveries: list[dict] | None = None) -> list[dict]:
    approved_switches = {
        (
            row["to_logical_vehicle_id"],
            row["from_raw_track_ids"],
            row["to_raw_track_ids"],
        )
        for row in accepted_cross_raw_recoveries or []
        if row.get("review_status") == "AUTO_MERGE_APPLIED"
    }
    by_logical: dict[str, list[dict]] = defaultdict(list)
    for row in logical_rows:
        if row.get("association_status") != "accepted":
        ...
        for previous, current in zip(ordered, ordered[1:]):
            ...
            if previous["raw_track_id"] != current["raw_track_id"]:
                ...
                approved_switch = (
                    ...
                    current["raw_track_id"],
                ) in approved_switches
                if not approved_switch:
                    if ratio > 2.0:
...
def apply_cross_raw_recovery_merges(
    ...
    review_rows = build_cross_raw_recovery_review(logical_rows)
    next_best_margin_by_from = {
        row["from_logical_vehicle_id"]: float(row["best_candidate_margin"])
        for row in review_rows
        if row.get("candidate_rank") == "2"
    }
    ...
    for row in review_rows:
        copy = dict(row)
        from_id = row["from_logical_vehicle_id"]
        to_id = row["to_logical_vehicle_id"]
        from_rows = accepted_by_logical.get(from_id, [])
        to_rows = accepted_by_logical.get(to_id, [])
        competitor_count = int(float(row.get("competing_candidate_count") or 0))
        best_margin = next_best_margin_by_from.get(from_id, 0.0)
        ...
        is_mergeable = (
            ...
            and is_clear_best
            and not logical_ids_temporally_overlap(logical_rows, from_id, to_id)
        )
        if is_mergeable and from_id not in merge_map:
            merge_map[from_id] = to_id
            copy["review_status"] = "AUTO_MERGE_APPLIED"
            copy["reason"] = f"{copy['reason']}|short_fragment_to_mature_recovery"
        output_review_rows.append(copy)
    ...
    return output, output_review_rows
...
def build_logical_vehicle_consistency(
    ...
    purity_report = build_identity_purity_report(logical_tracks, cross_raw_recovery_review)
