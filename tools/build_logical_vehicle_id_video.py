...
def review_status_token(row: dict) -> str:
    ...
    tokens = []
    if association_status == "duplicate_suppressed":
        tokens.append("DUP")
    elif association_status == "fragment_suppressed":
        tokens.append("FRAG")
    elif association_status not in {"accepted", "manual_review_applied"}:
        tokens.append("LINK?")
    if final_gate_status == "REVIEW_ONLY_IF_UNCERTAIN":
        tokens.append("REVIEW")
    elif final_gate_status == "AUTO_EXCLUDE":
        tokens.append("EXCLUDE")
    if row.get("quality_status") == "RISK_REVIEW":
        tokens.append("QUALITY")
    return " ".join(tokens) if tokens else association_status or "accepted"


def detection_label(row: dict, mode: str = "final") -> str:
    if mode == "final":
        return final_detection_label(row)
    logical_id = row.get("logical_vehicle_id", "").strip()
    raw_id = row.get("raw_track_id", "").strip()
    if not logical_id or not raw_id:
        raise ValueError(f"{mode} render row is missing logical_vehicle_id or raw_track_id")
    class_name = row.get("class_name", "")
    confidence = float(row.get("confidence") or 0.0)
    if mode == "debug":
        status = row.get("association_status", "")
    elif mode == "review":
        status = review_status_token(row)
    else:
        raise ValueError(f"Unknown render mode: {mode}")
    return f"{logical_id}/{raw_id} {status} {class_name} {confidence:.2f}"
...
def color_identifier(row: dict) -> str:
    return final_detection_label(row)


def final_color_map(rows: list[dict], mode: str = "final") -> dict[str, tuple[int, int, int]]:
    identifiers = sorted({color_identifier(row) for row in rows})
...
def style_for_row(row: dict, colors: dict[str, tuple[int, int, int]], mode: str = "final") -> tuple[int, int, int]:
    return colors[color_identifier(row)]
...
def draw_detection(draw, row: dict, colors: dict[str, tuple[int, int, int]], font, mode: str = "final") -> None:
...
def render_final_logical_vehicle_video(
    ...
    colors = final_color_map(logical_rows, mode)
    ...
    while True:
        ...
        for row in grouped.get(frame_id, []):
            draw_detection(draw, row, colors, label_font, mode)
