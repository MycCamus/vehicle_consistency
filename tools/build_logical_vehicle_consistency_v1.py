...
def debug_render_rows(logical_rows: list[dict]) -> list[dict]:
    return [
        row
        for row in logical_rows
        if row.get("association_status") == "accepted"
...
def review_render_rows(logical_rows: list[dict]) -> list[dict]:
    return [
        row
        for row in logical_rows
        if row.get("association_status") != "accepted"
        or row.get("final_gate_status") != "AUTO_KEEP"
        or row.get("quality_status") == "RISK_REVIEW"
    ]


def attach_quality_status(logical_rows: list[dict], target_quality_report: list[dict]) -> list[dict]:
    quality_by_id = {row["logical_vehicle_id"]: row for row in target_quality_report}
    output = []
    for row in logical_rows:
        copy = dict(row)
        quality = quality_by_id.get(copy["logical_vehicle_id"], {})
        copy["quality_status"] = quality.get("quality_status", "")
        copy["risk_reasons"] = quality.get("risk_reasons", "")
        output.append(copy)
    return output
...
def build_outputs(
    ...
    render_tracks = attach_quality_status(outputs.logical_tracks, outputs.target_quality_report)
    final_rows = final_render_rows(render_tracks)
    debug_rows = debug_render_rows(render_tracks)
    review_rows = review_render_rows(render_tracks)
    if render_videos:
        render_final_logical_vehicle_video(
            clip_path=clip_path,
        ...
        render_final_logical_vehicle_video(
            clip_path=clip_path,
            logical_rows=debug_rows,
            output_path=video_paths["debug"],
            fps=fps,
            mode="debug",
        )
        render_final_logical_vehicle_video(
            clip_path=clip_path,
            logical_rows=review_rows,
            output_path=video_paths["review"],
            fps=fps,
            mode="review",
        )
