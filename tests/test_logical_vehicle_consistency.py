...
class LogicalVehicleConsistencyTest(unittest.TestCase):
    ...
    def test_association_reports_successor_conflict_when_best_target_is_already_used(self):
        rows = [
            detection(0, 1, 0),
            detection(1, 1, 1),
            detection(1, 2, 8),
        ...
        tracklets = build_tracklets(
            rows,
            allowed_track_ids={"mot_0001", "mot_0002", "mot_0003"},
            max_gap_frames=1,
        )
        assoc = associate_tracklets(tracklets, max_gap_frames=4, max_link_distance_px=20.0)

        self.assertEqual(
            {(row["from_tracklet_id"], row["to_tracklet_id"]) for row in assoc.accepted_links},
            {("mot_0001_tl01", "mot_0003_tl01")},
        )
        conflict_rows = [
            row
            for row in assoc.ambiguous_links
            if row["from_tracklet_id"] == "mot_0002_tl01" and row["to_tracklet_id"] == "mot_0003_tl01"
        ]
        self.assertEqual(len(conflict_rows), 1)
        self.assertEqual(conflict_rows[0]["link_status"], "REJECT_USED_TO_CONFLICT")
        self.assertEqual(conflict_rows[0]["review_status"], "PENDING_REVIEW")
    ...
    def test_cross_raw_recovery_merges_short_fragment_into_strong_mature_continuation(self):
        logical_rows = []
        for frame in range(48):
            logical_rows.append(
                {
                    **detection(frame, 588, 100 + frame * 0.8, cy=50),
                    "logical_vehicle_id": "lv_0039",
                    "raw_track_id": "mot_0588",
                    "tracklet_id": "mot_0588_tl01",
                    "source": "detected",
        ...
        for frame in range(88, 188):
            logical_rows.append(
                {
                    **detection(frame, 621, 138 + (frame - 88) * 0.7, cy=52),
                    "logical_vehicle_id": "lv_0040",
                    "raw_track_id": "mot_0621",
                    "tracklet_id": "mot_0621_tl01",
                    "source": "detected",
                    "association_status": "accepted",
                }
            )
        for frame in range(89, 189):
            logical_rows.append(
                {
                    **detection(frame, 622, 165 + (frame - 89) * 0.7, cy=78),
                    "logical_vehicle_id": "lv_0041",
                    "raw_track_id": "mot_0622",
                    "tracklet_id": "mot_0622_tl01",
                    "source": "detected",
                    "association_status": "accepted",
                }
            )

        output_rows, review_rows = apply_cross_raw_recovery_merges(logical_rows)
        ...
        self.assertNotIn("lv_0039", accepted_ids)
        self.assertIn("lv_0040", accepted_ids)
        self.assertIn("lv_0041", accepted_ids)
        self.assertTrue(
            all(
                row["logical_vehicle_id"] == "lv_0040"
                for row in output_rows
                if row["raw_track_id"] == "mot_0588"
            )
        )
        applied_rows = [row for row in review_rows if row["review_status"] == "AUTO_MERGE_APPLIED"]
        self.assertEqual(len(applied_rows), 1)
        self.assertEqual(applied_rows[0]["from_logical_vehicle_id"], "lv_0039")
        self.assertEqual(applied_rows[0]["to_logical_vehicle_id"], "lv_0040")

    def test_cross_raw_recovery_keeps_merged_mature_path_final_keep(self):
        logical_rows = []
        for frame in range(48):
            logical_rows.append(
                {
        ...
        outputs = build_logical_vehicle_consistency(
            detections=[
                {
                    "track_id": row["raw_track_id"].replace("mot_", ""),
            ...
            final_targets=[{"track_id": "mot_0588"}, {"track_id": "mot_0621"}, {"track_id": "mot_0622"}],
            max_gap_frames=10,
            max_link_distance_px=80.0,
            max_iou=0.85,
        )
        ...
        merged_summary = next(
            row
            for row in outputs.logical_vehicle_summary
            if row["raw_track_ids"] == "mot_0588|mot_0621"
        )
        gate_by_id = {row["logical_vehicle_id"]: row for row in outputs.final_target_gate}
        merged_gate = gate_by_id[merged_summary["logical_vehicle_id"]]
        self.assertEqual(merged_gate["final_gate_status"], "AUTO_KEEP")
        self.assertEqual(merged_gate["purity_status"], "PURITY_PASS")

    def test_cross_raw_recovery_keeps_weak_candidate_for_review(self):
        logical_rows = []
        for frame in range(120):
            logical_rows.append(
                {
                    **detection(frame, 392, 100 + frame, cy=100),
                    "logical_vehicle_id": "lv_0021",
                    "raw_track_id": "mot_0392",
                    "tracklet_id": "mot_0392_tl01",
                    "source": "detected",
        ...
        for frame in range(160, 260):
            logical_rows.append(
                {
                    **detection(frame, 570, 330 + (frame - 160) * 0.8, cy=150),
                    "logical_vehicle_id": "lv_0036",
                    "raw_track_id": "mot_0570",
                    "tracklet_id": "mot_0570_tl01",
                    "source": "detected",
        ...
        self.assertEqual(accepted_ids, {"lv_0021", "lv_0036"})
        self.assertEqual(review_rows[0]["review_status"], "REVIEW_CROSS_RAW_RECOVERY")
